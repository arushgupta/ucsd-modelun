# class Secretariat < ActiveRecord::Base
	...
	validates :position, :presence => true,:format => { :with => /\A[a-zA-Z_\-]+\z/},length: { maximum: 100}
	validates :name,:presence => true,:format => { :with => /\A[a-zA-Z_\-]+\z/},length: { maximum: 100}
	validates :college,:presence => true,:format => { :with => /\A[a-zA-Z_\-]+\z/},length: { maximum: 100}
	validates :major,:presence => true,:format => { :with => /\A[a-zA-Z_\-]+\z/},length: { maximum: 100}
